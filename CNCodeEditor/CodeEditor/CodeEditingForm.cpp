...
BEGIN_MESSAGE_MAP(CodeEditingForm, CFrameWnd)
	ON_WM_CREATE()
	ON_WM_CLOSE()
	ON_WM_SIZE()
	ON_WM_SETFOCUS()
...
void CodeEditingForm::OnSize(UINT nType, int cx, int cy) {
	CRect rect;
	this->GetClientRect(rect);
...
void CodeEditingForm::OnSimpleCommandRange(UINT uID) {
	CommandFactory commandFactory(this);
	Command* command = commandFactory.Make(uID);
	if (command != NULL) {
		command->Execute();
...
LRESULT CodeEditingForm::OnThreadNotify(WPARAM wParam, LPARAM lParam) {
	if (this->printStateDialog != NULL) {
